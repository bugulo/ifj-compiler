...
void free_parser(ParserData *data) {
    ...
    for(unsigned i = 0; i < data->strings->length; i++)
        free(vectorGet(data->strings, i));

    for(unsigned i = 0; i < data->vectors->length; i++)
        vectorFree((Vector*) vectorGet(data->vectors, i));

    for(unsigned i = 0; i < data->type_vectors->length; i++) {
        removeFuncTypes((Vector*) vectorGet(data->type_vectors, i));
        vectorFree((Vector*) vectorGet(data->type_vectors, i));
    }

    for(unsigned i = 0; i < data->scopes->length; i++)
        htab_free((htab_t*) vectorGet(data->scopes, i));

    vectorFree(data->scopes);
    vectorFree(data->vectors);
    vectorFree(data->type_vectors);
...
void ruleProgram(ParserData *data) {
    ...
    if(data->isFirstScan) {
        if(!isFuncDefined(data->table, "main"))
            ...
            vectorPush(data->vectors, (void *) empty);
...
void ruleFunc(ParserData *data) {
    ...
    vectorPush(data->vectors, (void *) params);
    vectorPush(data->vectors, (void *) names);
    vectorPush(data->vectors, (void *) returns);
    ...
    if(data->isFirstScan) {
        ...
        if(string_compare(&data->function, "main"))
            ...
            if(getFuncReturnTypes(data->table, data->function.ptr)->length == 0) {
                Vector *empty = vectorInit();
                vectorPush(data->vectors, (void *) empty);
                return_function(empty, data->scopes);
...
void ruleStatBody(ParserData *data, Token id) {
    ...
    if(data->token.type == TOKEN_DECLARATION) {
        ...
        vectorPush(data->type_vectors, (void *) types);
        vectorPush(data->vectors, (void *) names);
        ...
        if(string_compare(&id.value.s, "print")) {
            ...
            Vector *empty = vectorInit();
            vectorPush(data->vectors, (void *) empty);
            call_function(id.value.s.ptr, names, empty, data->scopes);
        }
    } else {
        push_token(data, data->token);
        ...
        vectorPush(data->type_vectors, (void *) ltypes);
        vectorPush(data->vectors, (void *) lnames);
        ...
        if(result.isFunc) {
            ...
            vectorPush(data->type_vectors, (void *) rtypes);
            vectorPush(data->vectors, (void *) rnames);
...
bool ruleValues(ParserData *data, Vector *names, Vector *types) {
    ...
    if(data->token.type != TOKEN_IDENTIFIER && data->token.type != TOKEN_NUMBER_FLOAT && data->token.type != TOKEN_NUMBER_INT && data->token.type != TOKEN_STRING)
        ...
        if(data->token.type == TOKEN_IDENTIFIER) {
            if(string_compare(&data->token.value.s, "_"))
                throw_error_parser(data, OTHER_SEMANTIC_ERROR, "%s", "Cannot use _ in function call");
...
void register_functions(ParserData *data) {
    ...
    vectorPush(data->vectors, (void *) inputs_returns);
    addFuncType(inputs_returns, STRING);
    addFuncType(inputs_returns, INTEGER); 
    defineFunc(data->table, "inputs", empty, inputs_returns);

    Vector *inputi_returns = vectorInit();
    vectorPush(data->vectors, (void *) inputi_returns);
    addFuncType(inputi_returns, INTEGER);
    addFuncType(inputi_returns, INTEGER);
    defineFunc(data->table, "inputi", empty, inputi_returns);

    Vector *inputf_returns = vectorInit();
    vectorPush(data->vectors, (void *) inputf_returns);
    addFuncType(inputf_returns, FLOAT);
    addFuncType(inputf_returns, INTEGER);
    defineFunc(data->table, "inputf", empty, inputf_returns);

    Vector *int2float_params = vectorInit();
    Vector *int2float_returns = vectorInit();
    vectorPush(data->vectors, (void *) int2float_params);
    vectorPush(data->vectors, (void *) int2float_returns);
    ...
    vectorPush(data->vectors, (void *) float2int_params);
    vectorPush(data->vectors, (void *) float2int_returns);
    ...
    vectorPush(data->vectors, (void *) len_params);
    vectorPush(data->vectors, (void *) len_returns);
    ...
    vectorPush(data->vectors, (void *) substr_params);
    vectorPush(data->vectors, (void *) substr_returns);
    ...
    vectorPush(data->vectors, (void *) ord_params);
    vectorPush(data->vectors, (void *) ord_returns);
    ...
    vectorPush(data->vectors, (void *) chr_params);
    vectorPush(data->vectors, (void *) chr_returns);
    ...
    defineFunc(data->table, "print", empty, empty);
}
...
void parse() {
    ...
    data.vectors = vectorInit();
    data.type_vectors = vectorInit();
